...
'''perform DTW separately on each 3D sub-block!'''
...
import time
from scipy.signal import argrelextrema
def ExtractHorizon (seismic_data, grid, path_info, events, event_spacing_parameter, start_horizon_tracker, print_output):
    ...
    for start_trace in start_horizon_tracker:
        ...
        print("selected peaks and troughs along starting trace:", amplitudes)
        ...
        for event in amplitudes:
            ...
            if print_output==True:
                print("status:", 'horizon number', it, ' time in minutes: ', t)
            it=it+1
...
def ShowHorizon (seismic_data, horizons, horizon_number, view, line_number, selem):
    ...
    if horizon_number == 'all':
        ...
        for i in range (0, len(horizons)):
            ...
            for p in reflector:
                ...
                y=p[1]
                z=p[2]
                correlated_cube[x,y,z]=num #+1 FOR Å FÅ EN IDE OM HITRATE
    else:
    ...
    plt.title('Autotracked horizon superimposed onto seismic')
    plt.ylabel('Samples')
    plt.tight_layout()
    plt.show()
...
def regrid_horizons_DTW(seismic_data, grid, grid_step, filtered_horizons_binary, dense_grid):
    horizontal_warp_distance=2*grid_step
    dense_path_info=[]
    start = time.time()
...
def interpolateHorizons (seismic_data,unwrapped3D, horizons, dense_grid_step, phase_criterion, vertical_criterion, horizontal_criterion):
